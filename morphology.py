...
def erode_once(img, kernel):
    ...
    for i in range(pad_h, img_padded.shape[0] - pad_h):
        for j in range(pad_w, img_padded.shape[1] - pad_w):
            ...
            eroded_img[i - pad_h, j - pad_w] = np.min(region * kernel)  # Fit condition using min value
...
def dilate_once(img, kernel):
    ...
    for i in range(pad_h, img_padded.shape[0] - pad_h):
        for j in range(pad_w, img_padded.shape[1] - pad_w):
            region = img_padded[i - pad_h:i + pad_h + 1, j - pad_w:j + pad_w + 1]
            dilated_img[i - pad_h, j - pad_w] = np.max(region * kernel)  # Hit condition using max value
...
def morphology_process(image_path, output_path):
    ...
    dilated = dilate(img, kernel, iterations=2)
    eroded = erode(dilated, kernel, iterations=2)
    ...
    plt.subplot(122), plt.imshow(eroded, cmap='gray'), plt.title('Processed'), plt.axis('off')
    plt.tight_layout()
    plt.show()
